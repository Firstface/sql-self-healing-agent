...
class LLMAdapter:
    ...
    def generate_structured(self, prompt: str, response_model: type[T], *, purpose: str, input_summary: str) -> T:
        return self.hook_manager.execute_llm_call(
            lambda schema_feedback: self.client.generate_structured(
                self._with_schema_feedback(prompt, schema_feedback), response_model
            ),
