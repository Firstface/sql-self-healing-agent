...
- [x] ReadLogTool 日志管道复用、SubAgent Request/独立预算/mini-loop 收敛（`01be026`）
- [x] Memory 排序/扫描预算/降级、在线 SubAgent、契约与安全验收收敛（已完成）
- [x] 新 FAILED/SUCCESS 在线主链接入 AgentRunner（`c5f8736`）
- [x] 删除旧固定 Workflow 在线入口并完成最终收敛（已完成）
...
- UNKNOWN 诊断可保守触发受治理 SubAgent；Memory 按命中/相关性/更新时间排序并限制 unknown 扫描；写入失败降级 SUCCESS_ACK
